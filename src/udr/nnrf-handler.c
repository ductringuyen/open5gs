...
bool udr_nnrf_handle_nf_status_notify(ogs_sbi_server_t *server,
...
void udr_nnrf_handle_nf_discover(
        ogs_sbi_session_t *session, ogs_sbi_message_t *message)
{
    ogs_sbi_nf_instance_t *nf_instance = NULL;
        ...
        if (NF_INSTANCE_IS_OTHERS(nf_instance->id)) {
            ...
            if (!handled) {
                ogs_error("[%s] ogs_sbi_nnrf_handle_nf_profile() failed",
                        nf_instance->id);
                UDR_NF_INSTANCE_CLEAR("NRF-discover", nf_instance);
                continue;
            }
            ...
            if (!OGS_SBI_NF_INSTANCE_GET(
                        udr_ue->nf_types, nf_instance->nf_type))
                ogs_sbi_nf_types_associate(udr_ue->nf_types,
                        nf_instance->nf_type, udr_nf_state_registered);
