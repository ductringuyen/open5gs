        ...
        ogs_sbi_object_t *sbi_object)
{
    ...
    if (!OGS_SBI_NF_INSTANCE_GET(sbi_object->nf_types,
        ...
        nf = ogs_sbi_nf_types_associate(sbi_object->nf_types,
                sbi_object->nf_type, amf_nf_state_registered);
    ...
    if (nf == false) {
        ogs_warn("[%s] Try to discover",
                OpenAPI_nf_type_ToString(sbi_object->nf_type));
        ogs_timer_start(sbi_object->client_wait_timer,
        ...
        ogs_nnrf_disc_send_nf_discover(
            ...
            sbi_object->nf_type, OpenAPI_nf_type_AMF, sbi_object);
        ...
        ogs_sbi_request_t *(*build)(amf_ue_t *amf_ue, void *data))
{
    ...
    if (!nf_instance)
        nf_instance = find_or_discover_nf_instance(&amf_ue->sbi);
